    ...
    if (limit == '' || offset == '') {
      ...
      if (categorySearch == 'หมวดหมู่ทั้งหมด') {
        ...
        resultSearch = await sequelize.query(
          `select p.id productId, p.product_name productName, p.product_unitprice productUnitprice, ps.alreadysold , ps.inventory, pi.image1 , p.created_at createdAt, dis.id discountsId, dis.discounts discounts   from ((product_item p join product_stock ps on p.stock_id = ps.id)  join product_images pi on p.images_id = pi.id) left join discounts dis on p.discounts_id = dis.id where ${resultKeyword} order by ${orderBy} ${limit} ${offset}`,
          {
      ...
      if (categorySearch == 'หมวดหมู่ทั้งหมด') {
        resultSearch = await sequelize.query(
          `select p.id productId, p.product_name productName, p.product_unitprice productUnitprice, ps.alreadysold , ps.inventory, pi.image1 , p.created_at createdAt, dis.id discountsId, dis.discounts discounts  from ((product_item p join product_stock ps on p.stock_id = ps.id)  join product_images pi on p.images_id = pi.id) left join discounts dis on p.discounts_id = dis.id  where ${resultKeyword} order by ${orderBy} limit ${limit} offset ${offset}`,
          {
            type: QueryTypes.SELECT,
          }
        );
      ...
      if (categorySearch !== 'หมวดหมู่ทั้งหมด') {
        resultSearch = await sequelize.query(
          `select p.id productId, p.product_name productName, p.product_unitprice productUnitprice, ps.alreadysold , ps.inventory, pi.image1 , pc.category_name categoryName, p.created_at createdAt, dis.id discountsId, dis.discounts discounts   from (((product_item p join product_stock ps on p.stock_id = ps.id)  join product_images pi on p.images_id = pi.id) join product_category pc on p.category_id = pc.id) left join discounts dis on p.discounts_id = dis.id  where ${resultKeyword} having ${resultCategoryName} order by ${orderBy} limit ${limit} offset ${offset}`,
          {
            type: QueryTypes.SELECT,
          }
        );
