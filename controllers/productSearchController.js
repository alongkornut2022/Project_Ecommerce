    ...
    if (limit == '' || offset == '') {
      ...
      if (categorySearch == 'หมวดหมู่ทั้งหมด') {
        ...
        resultSearch = await sequelize.query(
          `select p.id productId, p.product_name productName, p.product_unitprice productUnitprice, ps.alreadysold , ps.inventory, pi.image1 , p.created_at createdAt  from (product_item p join product_stock ps on p.stock_id = ps.id)  join product_images pi on p.images_id = pi.id where ${resultKeyword} order by ${orderBy} ${limit} ${offset}`,
          {
      ...
      if (categorySearch == 'หมวดหมู่ทั้งหมด') {
        resultSearch = await sequelize.query(
          `select p.id productId, p.product_name productName, p.product_unitprice productUnitprice, ps.alreadysold , ps.inventory, pi.image1 , p.created_at createdAt  from (product_item p join product_stock ps on p.stock_id = ps.id)  join product_images pi on p.images_id = pi.id where ${resultKeyword} order by ${orderBy} limit ${limit} offset ${offset}`,
          {
            type: QueryTypes.SELECT,
          }
        );
